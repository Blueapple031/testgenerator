...
### 1.1 목표
대학교 강의자료 PDF와 과거 시험자료를 업로드하면, 서버가 문서를 분석하고 강의 내용에 근거한 시험 대비 문제집을 자동 생성하는 웹 애플리케이션을 개발한다. 사용자는 브라우저에서 PDF를 업로드하고 문제 생성 조건을 입력하며, 생성된 문제집을 PDF로 내려받거나 웹에서 바로 문제를 풀 수 있다. PWA(Progressive Web App)를 적용하여 앱과 유사한 사용 경험을 제공한다.
...
생성 결과는 다음 두 형태로 제공한다.

- 구조화된 JSON: 문제, 정답, 해설, 출처, 개념 태그
- LaTeX 기반 PDF: 출력 및 다운로드 가능한 시험 대비 문제집
...
9. LaTeX 기반 PDF 문제집 생성
10. 웹 애플리케이션에서 업로드, 상태 확인, 문제집 생성, PDF 다운로드
...
| 구성 요소 | 역할 |
|---|---|
| Next.js Frontend | PDF 업로드, 시험 범위 선택, 생성 옵션 입력, 실시간 상태 조회, 온라인 풀이·타이머, 문제 편집, PDF 다운로드, PWA |
...
| MinIO Container | S3 호환 오브젝트 스토리지, 원본 PDF와 결과 PDF 저장 |
...
- 원본 PDF와 생성 PDF는 MinIO에 저장하고 Docker volume 단위로 백업한다.
...
서버 → SSE 스트림:
  ...
  data: {"stage": "RENDERING", "progress": 90, "message": "PDF 컴파일 중..."}
...
→ 최종 확인 후 PDF 재생성 (rebuild-pdf API)
```
...
| generated_exam | 생성 시험 메타데이터 및 PDF 위치 |
...
```text
PENDING → GENERATING → RENDERING → COMPLETED
                         ↘ FAILED
```
...
│   │   ├── exams.py               # 시험 JSON 조회, PDF 다운로드, 문제 편집·재생성
│   │   ├── solve.py               # 풀이 결과 제출
...
│   │   ├── latex_pdf_service.py   # LaTeX → PDF
...
| 미리보기·편집 | 생성 문제 확인, 개별 문제 수정·삭제·재생성 후 최종 PDF 생성 |
| 다운로드 | PDF 저장 (오프라인 학습용) |
...
| Method | Endpoint | 설명 |
|---|---|---|
| GET | `/api/exams/{examId}` | 전체 시험 JSON 조회 |
| GET | `/api/exams/{examId}/download` | PDF 다운로드 |
...
| POST | `/api/exams/{examId}/rebuild-pdf` | 편집 후 PDF 재생성 |
...
| Phase 5 | 3~4일 | LaTeX PDF 생성 + 다운로드 |
...
10. FastAPI 서버가 LaTeX 템플릿을 통해 PDF를 생성한다.
11. 결과 PDF가 MinIO에 저장되고 웹에서 다운로드된다.
